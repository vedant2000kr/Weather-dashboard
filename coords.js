...
// url = `https://api.weatherbit.io/v2.0/forecast/daily?city=Raleigh,NC&key=${apiKey}`;
...
continue_.addEventListener('click', () => {
    ...
    console.log(language, latitude, longitude, date, parameter, trigger);

    if (trigger == false) {

        let url = `https://api.weatherbit.io/v2.0/forecast/daily?&lat=${latitude}&lon=${longitude}&lang=${language}&key=${apiKey}`;
        fetch(url)
        ...
        let url = `https://api.weatherbit.io/v2.0/current?&lat=${latitude}&lon=${longitude}&lang=${language}&key=${apiKey}`;
        fetch(url)
            .then(response => response.json())
            .then((obj) => {
