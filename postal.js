...
continue_.addEventListener('click', () => {
    ...
    if (trigger == false) {
        ...
        let url = `https://api.weatherbit.io/v2.0/forecast/daily?&postal_code=${postal}&lang=${language}&key=${apiKey}`;
        fetch(url)
        ...
        let url = `https://api.weatherbit.io/v2.0/current?&postal_code=${postal}&lang=${language}&key=${apiKey}`;
        fetch(url)
            .then(response => response.json())
            .then((obj) => {
